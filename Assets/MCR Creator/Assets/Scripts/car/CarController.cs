...
public class CarController : MonoBehaviour
{
    ...
    public List<float>
        Wheel_X_RefLocalPosition =
            new List<float>(); // Save when the script Awake the local position of the 4 Wheel_X_Rotate. 
    ...
    public float
        BodyRotationValue =
            5; // use to move the car body. Fake Left and right movement. Custom Editor                        
    ...
    public bool
        b_CarMoveForward =
            true; // Know if the car move forward. Use to apply rotation when player is moving forward or backward
    ...
    public GameObject
        camTarget; // Use to initialized the Camera when GameStart Call by script Cam_Follow.cs on P1_Cam or P2_Cam
    ...
    void Awake()
    {
        ...
        {
            Wheel_X_RefLocalPosition.Add(Wheel_X_Rotate[i].transform.localPosition
                .y); // Save the wheel position reference
        }
    ...
    public void UpdateCarInputs()
    {
        ...
        */
        ...
        if (PlayerPrefs.GetString("Which_GameMode") == "OnlineMultiPlayer")
        {
            ...
            MCR.GameManager_MCR_Photon mPhoton =
 GameObject.Find("GM_Photon").GetComponent< MCR.GameManager_MCR_Photon>();
        ...
        GameObject
            cameraMainMenu =
                GameObject.FindGameObjectWithTag(
                    "CameraMainMenu"); // Use to prevent bug when car is used on Main Menu. We don't the car sound in this case
    ...
    public void RespawnTheCar()
    {
        ...
        {
            if (respawnManager)
                respawnManager.MCR_AddCarToRespawnList(gameObject.GetComponent<CarController>(),
    ...
    IEnumerator RepawnInitialization()
    {
        ...
        yield return new WaitUntil(() =>
            respawnManager.carCanGoToRespawnPosition(gameObject.GetComponent<CarController>()) == true);
        ...
        while (t < timeToMove)
        {
            ...
            if (!b_Pause)
            {
                ...
                TargetPos = Vector3.Lerp(TargetPos, targetPos + new Vector3(0, .5F, 0),
                    1f - Mathf.Cos(t * Mathf.PI * 0.5f));
                transform.position = TargetPos;

                transform.rotation = Quaternion.Lerp(transform.rotation, targetQuaternion,
                    1f - Mathf.Cos(t * Mathf.PI * 0.5f));
            }
        ...
        yield return new WaitUntil(
            () => respawnManager.carCanRespawn(gameObject.GetComponent<CarController>()) == false);
        yield return new WaitUntil(() =>
            respawnManager.UpdateCarToRespawn(gameObject.GetComponent<CarController>()) == true);
    ...
    void MCR_carV1()
    {
        if (!b_Pause || !b_CarIsRespawning && !b_Pause)
        {
            ...
            if (Input_Left != Input_Right)
            {
                ...
                {
                    ReachMaxRotation = Mathf.MoveTowards(ReachMaxRotation, 1,
                        Time.deltaTime * (CarRotationSpeed + offsetRotationForMobile));
                    btn_Rotation = Mathf.MoveTowards(btn_Rotation, -1,
                        Time.deltaTime * 6 * curveRotation.Evaluate(ReachMaxRotation));
                    turnDirection = -1;
                }
                else if (!b_AutoAcceleration && b_btn_Right && turnDirection != -1 // mobile
                         || !b_AutoAcceleration && Input.GetAxisRaw(Input_Right) >= .4F &&
                         turnDirection != -1 // gamepad
                ...
                )
                {
                    ...
                    turnDirection = 1;
                         ...
                         || !b_AutoAcceleration && Input.GetAxisRaw(Input_Left) == 0 &&
                         Input.GetAxisRaw(Input_Right) == 0 // gamepad
            ...
            if (rb.velocity.magnitude > .2f && (turnDirection == 1 && !b_AutoAcceleration ||
                                                turnDirection == 1 && b_AutoAcceleration &&
                                                Mathf.Abs(carAI.angle) > 10))
            {
                ...
                if (onlineNoFakeRotation)
                {
                    ...
                    tmpFakeBodyRotation = Mathf.MoveTowards(tmpFakeBodyRotation, -1 * (BodyRotationValue + 2),
                        10 * Time.deltaTime); // car AI
                else
                    tmpFakeBodyRotation =
                        Mathf.MoveTowards(tmpFakeBodyRotation, -1 * BodyRotationValue, 10 * Time.deltaTime); // player
            }
            else if (rb.velocity.magnitude > .2f && (turnDirection == -1 && !b_AutoAcceleration ||
                                                     turnDirection == -1 && b_AutoAcceleration &&
                                                     Mathf.Abs(carAI.angle) > 10))
            {
                if (onlineNoFakeRotation)
                {
                    ...
                    tmpFakeBodyRotation =
                        Mathf.MoveTowards(tmpFakeBodyRotation, BodyRotationValue + 2, 10 * Time.deltaTime); // car Ai
                else
                    tmpFakeBodyRotation =
                        Mathf.MoveTowards(tmpFakeBodyRotation, BodyRotationValue, 10 * Time.deltaTime); // Player
            }
            else
            {
                if (onlineNoFakeRotation)
                {
                    ...
                    tmpFakeBodyRotation = Mathf.MoveTowards(tmpFakeBodyRotation, 0, 10 * Time.deltaTime); // car AI
            ...
            for (int i = 0; i < RayCastWheels.Length; i++)
            {
                ...
                if (Physics.Raycast(RayCastWheels[i].transform.position, dir, out hit, dis, myLayerMask))
                {
                    ...
                    if (rb.velocity.magnitude < 2f && Input_Acceleration == 0)
                    {
                        if (rb.velocity.magnitude < .5f)
                        {
                            ...
                            Slide_01 = Mathf.MoveTowards(Slide_01, 0,
                                Time.deltaTime * .1f * curve_01.Evaluate(Slide_03));
                            Slide_02 = Mathf.MoveTowards(Slide_02, 1,
                                Time.deltaTime * .1f * curve_01.Evaluate(Slide_03));
                        }
                        else
                        {
                    ...
                    if (Slide_02 > 0)
                        Debug.DrawRay(RayCastWheels[i].transform.position,
                            (RayCastWheels[i].transform.up * Slide_01 + Vector3.up * Slide_02) * .25f, Color.cyan);
                    else
                        Debug.DrawRay(RayCastWheels[i].transform.position,
                            (RayCastWheels[i].transform.up * Slide_01 + Vector3.up * Slide_02) * .25f, Color.yellow);
                    ...
                    if (hit.distance > .1)
                    {
                        if (i == 0 || i == 1)
                        {
                            Wheel_X_Rotate[i].transform.position = new Vector3(Wheel_X_Rotate[i].transform.position.x,
                                hit.point.y - .03f + offsetWheelFront, Wheel_X_Rotate[i].transform.position.z);
                            Wheel_X_Rotate[i].transform.localPosition =
                                new Vector3(0, Wheel_X_Rotate[i].transform.localPosition.y, 0);
                        }
                        else
                        {
                            Wheel_X_Rotate[i].transform.position = new Vector3(Wheel_X_Rotate[i].transform.position.x,
                                hit.point.y - .03f + offsetWheelRear, Wheel_X_Rotate[i].transform.position.z);
                            Wheel_X_Rotate[i].transform.localPosition =
                                new Vector3(0, Wheel_X_Rotate[i].transform.localPosition.y, 0);
                        }
                        ...
                        if (Wheel_X_Rotate[i].transform.localPosition.y <= Wheel_X_RefLocalPosition[i])
                        {
                            ...
                            tmpWheelX_LocalPosition = Mathf.MoveTowards(tmpWheelX_LocalPosition,
                                Wheel_X_RefLocalPosition[i], Time.fixedDeltaTime);
                ...
                else
                {
                    ...
                    float tmpWheelX_LocalPosition = Wheel_X_Rotate[i].transform.localPosition.y;
                    tmpWheelX_LocalPosition = Mathf.MoveTowards(tmpWheelX_LocalPosition, Wheel_X_RefLocalPosition[i],
                        Time.fixedDeltaTime); // init wheel poisition
                    Wheel_X_Rotate[i].transform.localPosition = new Vector3(0, tmpWheelX_LocalPosition, 0);
            ...
            if (NumberOfWheelThatTouchGround >= 2)
            {
                        ...
                        if (!raceIsFinished && !b_CountdownActivate)
                        {
                            ...
                            if (b_IsMine)
                            {
                                rb.AddForceAtPosition(
                                    rb.transform.forward * tmpmulti * Force *
                                    curveAcceleration.Evaluate(ReachMaxRotationAcc) * Input_Acceleration, tmpVect,
                                    ForceMode.Force);
                            }
                        }
                    }
                ...
                {
                    tmpPitch = .5F + MaxAudioPitch * Speed /
                               (MaxSpeed + randomSpeedOffset + offsetSpeedDifficultyManager + offsetSpeedForMobile);
                } // Case : race is starting
                else if (b_CountdownActivate && Input_Acceleration >= 0)
                {
                    // Case : countdown
                    tmpPitch = .5F + (MaxAudioPitch * MaxSpeed * Input_Acceleration) /
                               (MaxSpeed + randomSpeedOffset + offsetSpeedDifficultyManager + offsetSpeedForMobile);
                }
                else if (b_CountdownActivate && Input_Acceleration < 0)
                {
            ...
            float VelocityDir =
                Vector3.Angle(rb.velocity, rb.transform.forward); // know if the car go backward or forward
            ...
            if (b_IsMine)
            {
                Quaternion deltaRotation =
                    Quaternion.Euler(eulerAngleVelocity * tmpAngleRotation * tmprotate * Time.fixedDeltaTime *
                                     btn_Rotation * DirBackward_ * CoeffRotionIfJump);
        ...
        {
            if (rb.velocity.magnitude < 0.1f && NumberOfWheelThatTouchGround == 0 && !b_AutoAcceleration &&
            ...
            if (Physics.Raycast(rb.transform.position, -Vector3.up, out hit3, 10))
            {
                ...
                for (var i = 0; i < gripSurface.listOfSurface.Count; i++)
                {
                    if (hit3.transform.tag == gripSurface.listOfSurface[i]._Tag) // Find the surface type
                    {
                        ...
                        {
                            CoeffZWhenCarIsSlow =
                                gripSurface.listOfSurface[i]
                                    .CoeffZWhenCarIsSlow; // 1.5 = the car stop very quickly / 5 = the car stop slowly                    
                            GripForward =
                                gripSurface.listOfSurface[i].GripForward; // Reduce the speed of the car 0 to 1
                            BrakeForce = gripSurface.listOfSurface[i].BrakeForce; // Reduce or increase break force 
                            SlideCoeff =
                                gripSurface.listOfSurface[i]
                                    .SlideCoeff; // Slide coefficient .1f very little slide / .001 very long slide
                            //if (!carAI.enabled)
                            SlideEulerAngleY =
                                gripSurface.listOfSurface[i].SlideEulerAngleY; // add rotation to the wheel  
                        }
