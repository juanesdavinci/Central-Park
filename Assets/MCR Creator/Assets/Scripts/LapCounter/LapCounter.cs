...
//public List<int> 											carPositionAtEnd = new List<int> ();		// Know if a car is on 1 2 3 4 position in a race when race is finished
    ...
    private UnityStandardAssets.Utility.WaypointCircuit
        Track; // A reference to the waypoint-based route we should follow
    ...
    public Transform CoinsManager;
    public Transform TurboManager;
...
// --> Init list car and list carController. Call by the Game_Manager or at the beginning of this script if ''Test Mode'' is activated
    public void InitCar()
    {
        ...
        foreach (GameObject carFind in arrCars)
        {
            ...
            {
                car[carFind.GetComponent<CarController>().playerNumber - 1] =
                    carFind.GetComponent<CarPathFollow>(); // access component for each car
                carController[carFind.GetComponent<CarController>().playerNumber - 1] =
                    carFind.GetComponent<CarController>(); // access component for each car


                if (carFind.GetComponent<CarController>().playerNumber == 2 &&
                    carFind.GetComponent<CarAI>().enabled == false
                ) // Know if player 2 is manage by a human Player or CPU
...
// --> Pause
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.tag == "Car" && carPathFollow.iLapCounter < lapNumber + 1)
        {
            // A car finish a lap
            ...
            if (other.tag == "Car" && carPathFollow.iLapCounter >= lapNumber + 1)
            {
                ...
                for (var i = 0; i < car.Count; i++)
                {
                    ...
                    if (car[i] != null && car[i].gameObject == other.gameObject && !raceFinished[i])
                    {
						...
						MCR.GameManager_MCR_Photon GM_Photon =
 GameObject.Find("GM_Photon").GetComponent<MCR.GameManager_MCR_Photon>();
                        ...
                        else if ((i == 0 || i == 1) && !player2IsManageByCPU && carController[0] != null &&
                                 carController[0].raceIsFinished && carController[1] != null &&
                                 carController[1].raceIsFinished)
                        {
                            gameManager.RaceIsFinished();
                            CheckIfNextTrackMustBeUnlocked();
                        }
    ...
    {
        foreach (Transform coin in CoinsManager)
        {
    ...
    {
        foreach (Transform turbo in TurboManager)
        {
    ...
    {
        var cm = other.GetComponent<CarController>();
        if (cm == null) return false;

        return cm.playerNumber == 1;
    }
